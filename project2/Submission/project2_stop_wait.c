...
void print_pkt(char *action, struct pkt packet)
{
   ...
   int i;
   for (i = 0; i < 20 && packet.payload[i]; i++)
...
{
   // Unidirectional, so B is not outputing any message.
}
...
void A_input(struct pkt packet)
{
   ...
   if (packet.isACK == 1 && !compute_check_sum(packet))
   {
      stoptimer(A);
      ...
      { /* NAK */
         if (DEBUG)
            print_pkt("NAK Received, Resent at A", waiting_packet_A);
         tolayer3(A, waiting_packet_A);
         starttimer(A, TIME_OUT);
      }
   }
   else
   {
      // retransmit
      if (DEBUG)
         print_pkt("Corrupted ACK/NACK, Resent at A", waiting_packet_A);
      stoptimer(A);
      tolayer3(A, waiting_packet_A);
      starttimer(A, TIME_OUT);
   }
...
{
   if (DEBUG)
      print_pkt("Resent at A", waiting_packet_A);
   tolayer3(A, waiting_packet_A);
   starttimer(A, TIME_OUT);
}
...
void B_input(packet) struct pkt packet;
{
   ...
   if (packet.seqnum == seq_expect_recv_B)
   {
      ...
      /* Send ACK to A side */
      waiting_packet_B.isACK = 1;
      waiting_packet_B.acknum = packet.seqnum;
      waiting_packet_B.payload[0] = 0;
      waiting_packet_B.checksum = 0;
      waiting_packet_B.checksum = compute_check_sum(waiting_packet_B);
      if (DEBUG)
         print_pkt("ACK sent from B", waiting_packet_B);
      tolayer3(B, waiting_packet_B);
   }
   else
   {
      if (DEBUG)
         print_pkt("Packet Already Accpeted, Resent ACK from B", waiting_packet_B);
      tolayer3(B, waiting_packet_B);
   }
}
...
{
   // The sender side will take care of the timer control.
}
